...
<div class="px-4 sm:px-6 lg:px-8 max-w-4xl">
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="pin" id="pin" maxlength="6" pattern="[0-9]{6}"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500"
                           value="{{ old('pin', $user->pin) }}" placeholder="000000">
                </div>
            ...
            <div id="permissions-section" class="bg-white shadow rounded-lg p-6 {{ old('role', $user->role) == 'admin' ? 'hidden' : '' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-2">
                    @php
                        ...
                        $user_permissions = is_array($user->permissions) ? $user->permissions : [];
                    @endphp
                    ...
                    @foreach($available_permissions as $key => $label)
                        <div class="flex items-center">
                            <input type="checkbox" name="permissions[]" value="{{ $key }}" id="perm_{{ $key }}"
                                   ...
                                   {{ in_array($key, old('permissions', $user_permissions)) ? 'checked' : '' }}>
