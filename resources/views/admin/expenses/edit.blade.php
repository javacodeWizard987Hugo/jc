...
<div class="px-4 sm:px-6 lg:px-8 max-w-2xl">
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route($routePrefix . '.expenses.update', $expense) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-5">
                    ...
                    <div>
                        ...
                        <input type="date" name="credit_repay_date" id="credit_repay_date"
                               ...
                               value="{{ old('credit_repay_date', $expense->credit_repay_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="cheque_number" id="cheque_number"
                                           ...
                                           value="{{ old('cheque_number', $expense->cheque_number) }}" placeholder="Enter cheque number">
                                </div>
                                <div>
                                    ...
                                    <select name="cheque_bank" id="cheque_bank"
                                        ...
                                        @foreach($banks as $bank)
                                            <option value="{{ $bank }}" {{ old('cheque_bank', $expense->cheque_bank) == $bank ? 'selected' : '' }}>{{ $bank }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input type="date" name="cheque_date" id="cheque_date"
                                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                                           value="{{ old('cheque_date', $expense->cheque_date?->format('Y-m-d')) }}">
                                </div>
                                <div>
                                    ...
                                    <input type="date" name="cheque_repay_date" id="cheque_repay_date"
                                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                                           value="{{ old('cheque_repay_date', $expense->cheque_repay_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="account_holder_name" id="account_holder_name"
                                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                                           value="{{ old('account_holder_name', $expense->account_holder_name) }}" placeholder="Enter account holder name">
                                </div>
                                <div>
                                    <label for="account_number" class="block text-sm font-medium text-gray-700 mb-1">Account Number *</label>
                                    <input type="text" name="account_number" id="account_number"
                                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                                           value="{{ old('account_number', $expense->account_number) }}" placeholder="Enter account number">
                                </div>
                                <div>
                                    ...
                                    <select name="account_bank" id="account_bank"
                                        ...
                                        <option value="">Select Bank</option>
                                        @foreach($banks as $bank)
                                            <option value="{{ $bank }}" {{ old('account_bank', $expense->account_bank) == $bank ? 'selected' : '' }}>{{ $bank }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="account_branch" id="account_branch"
                                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                                           value="{{ old('account_branch', $expense->account_branch) }}" placeholder="Enter branch name">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="mb-4" id="recurring_days_div" style="display: {{ old('is_recurring', $expense->is_recurring) ? 'block' : 'none' }};">
                ...
                <input type="number" name="recurring_days" id="recurring_days" min="1"
                       class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500"
                       value="{{ old('recurring_days', $expense->recurring_days) }}" placeholder="e.g., 30 for monthly">
            </div>
