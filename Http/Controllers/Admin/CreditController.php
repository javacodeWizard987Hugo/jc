...
class CreditController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.credits.index', compact('credits', 'customers'));
    }
    ...
    public function recordPayment(Request $request, CustomerCredit $credit)
    {
        $validated = $request->validate([
            'payment_amount' => 'required|numeric|min:0.01|max:' . $credit->outstanding_amount,
        ...
        try {
            ...
            $newPaidAmount = $credit->paid_amount + $paymentAmount;
            $newOutstanding = $credit->outstanding_amount - $paymentAmount;

            // Update credit record
            $credit->paid_amount = $newPaidAmount;
            $credit->outstanding_amount = $newOutstanding;
            ...
            $credit->notes = ($credit->notes ? $credit->notes . "\n" : '') . 
                           "Payment: Rs. {$paymentAmount} on {$validated['payment_date']}. " . 
                           ($validated['notes'] ?? '');
            $credit->save();
            ...
            AuditLog::log('credit_payment', 
                "Payment of Rs. {$paymentAmount} recorded for credit #{$credit->id} (Customer: {$customer->name})", 
                $credit);
